...
internal static class CatalogCategoryScope
{
        ...
        CancellationToken ct)
    {
        ...
        var applicationTypeLookupId = await CatalogApplicationVertical.ResolveApplicationTypeLookupIdForProductTypeAsync(
            db, tenantId, productTypeId, ct);

        var allowedDepartmentIds = CatalogApplicationVertical.DepartmentIdsForApplicationType(
            departmentRows,
            applicationTypeLookupId);
        ...
        {
            if (applicationTypeLookupId is not null
                && string.Equals(app.Id, applicationTypeLookupId, StringComparison.Ordinal))
                continue;
        ...
        bool includeDepartmentCategoryShell)
    {
        ...
        if (allowedDepartmentIds.Count > 0)
        {
            foreach (var c in allCats)
            {
                ...
                if (string.IsNullOrEmpty(dept) || !allowedDepartmentIds.Contains(dept))
                    continue;
                scope.Add(c.Id);
            }
        }

        if (productCategoryIdsByType.TryGetValue(productTypeId, out var fromProducts))
        {
            foreach (var categoryId in fromProducts)
            {
                var cat = allCats.FirstOrDefault(c => c.Id == categoryId);
                if (cat is null) continue;
                var dept = cat.ParentValueId;
                if (allowedDepartmentIds.Count > 0
                    && (string.IsNullOrEmpty(dept) || !allowedDepartmentIds.Contains(dept)))
                    continue;
                scope.Add(categoryId);
            }
        }

        if (includeDepartmentCategoryShell && allowedDepartmentIds.Count > 0)
        {
            foreach (var c in allCats)
            {
                var dept = c.ParentValueId;
        ...
        ExpandParentValueIdSiblings(scope, allCats, allowedDepartmentIds);
        AddMerchandisingDescendants(scope, allCats);
        RestrictToAllowedDepartments(scope, allCats, allowedDepartmentIds);
    ...
    private static void RestrictToAllowedDepartments(
        HashSet<string> scope,
        List<LookupValue> allCats,
        HashSet<string> allowedDepartmentIds)
    {
        if (allowedDepartmentIds.Count == 0) return;
        ...
        HashSet<string> allowedDepartmentIds)
    {
        ...
        {
            if (allowedDepartmentIds.Count > 0 && !allowedDepartmentIds.Contains(pv!))
                continue;
