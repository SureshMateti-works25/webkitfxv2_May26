/**
 * Smoke tests for Catalog.Api: health, signup, login, catalogue reads.
 *
 * Usage:
 *   node scripts/catalog-api-smoke.mjs           # full flow (register → login → catalog)
 *   node scripts/catalog-api-smoke.mjs auth      # health + register + login only
 *   node scripts/catalog-api-smoke.mjs catalog   # catalog GETs + optional login if SMOKE_EMAIL set
 *
 * Env:
 *   CATALOG_API_URL   default http://localhost:5055
...
const BASE = (process.env.CATALOG_API_URL ?? "http://localhost:5055").replace(/\/$/, "");
...
async function testRegister(email) {
  const res = await fetch(`${BASE}/api/v1/auth/register`, {
    ...
    body: JSON.stringify({
      ...
      password: PASSWORD,
      role: "shopper",
      profile: { source: "catalog-api-smoke" },
    }),
  });
  const data = await json(res);
  if (res.status === 409) {
...
async function main() {
  ...
  console.log(`Catalog.Api smoke — ${BASE} tenant=${TENANT} mode=${mode}\n`);
...
main().catch((e) => {
  ...
  if (refused) {
    console.error(`Cannot reach ${BASE}. Start Postgres (docker compose in services/catalog-api), then: npm run api:catalog`);
  } else {
    console.error(e);
  }
  process.exit(1);
