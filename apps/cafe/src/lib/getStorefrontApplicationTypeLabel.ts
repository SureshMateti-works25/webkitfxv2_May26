...
import { resolveApplicationTypeIdsForStorefront } from "./cafeLookupFilters.js";
...
export async function fetchStorefrontApplicationTypeLabel(): Promise<string> {
  ...
  try {
    ...
    const scopeIds = new Set(resolveApplicationTypeIdsForStorefront(rows));
    const configured = CATALOG_APPLICATION_TYPE_LOOKUP_ID.trim();
    const row =
      rows.find((r) => r.id === configured) ??
      rows.find((r) => scopeIds.has(r.id)) ??
      rows.find((r) => scopeIds.has(r.id.trim()));
