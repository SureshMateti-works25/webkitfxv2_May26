 ...
 * Commerce.Api accepts this on `productTypeId` query params and resolves to `products.product_type_id`.
 */
...
/** Same as {@link CATALOG_APPLICATION_TYPE_LOOKUP_ID} — stored on products/orders as `product_type_id`. */
export const STOREFRONT_RESOLVED_PRODUCT_TYPE_ID = CATALOG_APPLICATION_TYPE_LOOKUP_ID;
...
export type CatalogProductCard = {
  ...
  skuCodes: string[];
  /** Commerce.Api `ProductTypeId` on cards (canonical id, e.g. `pt_cafe`; storefront scope uses `application_type` lookup). */
