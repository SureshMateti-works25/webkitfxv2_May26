...
export function departmentBrowseGroupsByDepartments(
  ...
  options?: { includeEmptyDepartments?: boolean }
  ...
  for (const d of sorted) {
    ...
    const categories = cats
      ...
      .sort((a, b) => a.sortOrder - b.sortOrder || a.slug.localeCompare(b.slug));
    if (categories.length === 0 && !options?.includeEmptyDepartments) continue;
...
export { filterDepartmentsForStorefrontApplication } from "./cafeLookupFilters.js";

function departmentBrowseGroupsByApplicationType(
  cats: CatalogCategoryRow[],
  applicationTypeId: string,
  menuLabel = "Menu"
): DepartmentBrowseGroup[] {
  const categories = cats
    .filter((c) => (c.parentValueId ?? "").trim() === applicationTypeId)
    .sort((a, b) => a.sortOrder - b.sortOrder || a.slug.localeCompare(b.slug));
...
export function resolveStorefrontDepartmentGroups(
  ...
  const scope = buildStorefrontApplicationScope(applicationTypeRows, scopedDepts);
  const scopedCats = filterCategoriesForStorefrontApplication(cats, scope);
  const resolvedDeptRows = resolveDepartmentRowsForStorefront(scopedCats, bundle, scopedDepts);
  const deptGroups = departmentBrowseGroupsByDepartments(scopedCats, resolvedDeptRows, {
    includeEmptyDepartments: true,
  });
  const merchGroups = departmentBrowseGroupsMerchandising(scopedCats);
  ...
  const useDeptGrouping =
    deptGroups.length > 0 && deptTileCount >= Math.max(merchTileCount, Math.min(1, scopedCats.length));
  return {
