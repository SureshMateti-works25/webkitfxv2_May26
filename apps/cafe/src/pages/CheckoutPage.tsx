...
export function CheckoutPage() {
  ...
  const [tableCode, setTableCode] = useState(existing?.tableCode ?? "");
  ...
  const onContinue = () => {
    ...
    const draft: CheckoutDraft = {
      ...
      tableCode: serviceMode === "dine_in" ? table : undefined,
  ...
  return (
    <div className="checkout-page cart-page">
      <header className="cart-page__header">
        <h1>{String(checkoutCopy.title ?? "Checkout")}</h1>
        <p className="cart-page__lede">{String(checkoutCopy.lede ?? "")}</p>
      </header>
