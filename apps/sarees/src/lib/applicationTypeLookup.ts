...
export function applicationLookupValueIdFromProductTypeId(
  ...
  const match = rows.find((r) => {
    ...
    if (raw.includes("saree")) return code === "sr" || code.includes("saree") || label.includes("saree");
    if (raw.includes("grocery")) return code === "gr" || code.includes("grocery") || label.includes("grocery");
    if (raw.includes("cafe")) return code.includes("cafe") || label.includes("café") || label.includes("cafe");
    return false;
  });
  return match?.id ?? CATALOG_APPLICATION_TYPE_LOOKUP_ID;
}
